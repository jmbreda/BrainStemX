...
if 0
    ...
    for i = 1:length(node.name)
        if isempty(intersect(node.name{i},gene2motif.values))
            ...
            if 1%isempty(intersect(node.name{i},motif2go.keys))
                node.go{i} = {' '};
            else
    ...
    Graph = {'overlap=false','Goverlap=prism','fontname="Helvetica"',['label=' graph_label],'rankdir=LR'};
